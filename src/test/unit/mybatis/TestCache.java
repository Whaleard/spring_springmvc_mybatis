...
/**
 * MyBatis缓存查询的顺序
 *  1、先查询二级缓存，因为二级缓存范围更大。
 *  2、如果二级缓存没有命中，再查询一级缓存。
 *  3、如果一级缓存也没有命中，则查询数据库。
 *  4、SqlSession关闭之后，一级缓存中的数据会写入二级缓存。
 *
 * 一般项目中一级缓存基本等于“默认存在但感知不到”，二级缓存则大多不建议开。
 */
public class TestCache {
    ...
    /**
     * 二级缓存是Mapper级别，同一个Mapper接口下的所有SqlSession共享缓存数据，此后若再次执行相同的查询语句，结果就会从缓存中获取
     * 二级缓存开启的条件：
     *      1、在核心配置文件中，设置全局配置属性cacheEnable="true"，开启二级缓存
     *      2、在映射文件中设置标签<cache />
     *      3、二级缓存必须在SqlSession执行close（关闭）或commit（提交）之后有效
     *      4、查询的数据所转换的实体类类型必须实现序列化接口
     *  二级缓存失效情况：
     *      执行任意的增删改操作并commit，会清空整个namespace的二级缓存
     */
    @Test
    public void test02() {
        try {
